...
> Довідковий каталог початкового дизайну промптів. Він не є runtime-конфігурацією: актуальні системні промпти визначені безпосередньо в `app/api/agents/*/route.ts` і можуть відрізнятися від прикладів нижче.
